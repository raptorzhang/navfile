...
//		CodePress.syntaxHighlight('init');
	...
	insertSnippet : function(xSnippet) {
		if(!xSnippet || xSnippet=='') return;
		editor.focus();
		content = xSnippet;
		if(content.indexOf('$0')<0) content += cc;
		else content = content.replace(/\$0/,cc);
		this.insertCode(content,false);
		this.syntaxHighlight('generic');
		CodePress.findString();
	},
	
	insertSnippet2 : function(xSnippet) {
		if(!xSnippet || xSnippet=='') return;
		editor.focus();
		CodePress.findString();
		content = xSnippet.replace(/</g,'&lt;');
		content = content.replace(/>/g,'&gt;');
		if(content.indexOf('$0')<0) content += cc;
		else content = content.replace(/\$0/,cc);
		content = content.replace(/\n/g,'<br>');
		var pattern = new RegExp(cc,'gi');
		this.syntaxHighlight('snippets',pattern,content);
	},
...
//return(CodePress.getRangeAndCaret());
//return('index:'+editor.innerHTML.indexOf(cc));
	},

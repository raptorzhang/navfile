...
// 									-= Window =-
function WindowObject(xObjName,xHnd){
	xObj = document.getElementById(xObjName);
	this.xHnd = xHnd;
...
//	this.ScrollHeight = eval(xObjName + 'Scroll.style.height');
	...
	this.scroll = xObj.getAttribute('scrollBars');
	this.ID = xObj.id;
	xObj.setAttribute('xHnd',xHnd);
}
function regWindow(xObjName){
	if (xObjName==''){return;}
	Windows[WindowNext] = new WindowObject(xObjName,WindowNext);
	WindowNext++;
}
function preRegister(xObjName){
	PreReg = PreReg + xObjName + '\n';
}
function Register(){
	xObjs = PreReg.split('\n');
...
function DebugWindow(xObj){
  Debug.innerHTML = '<textarea style="border:none; width:' + (parseInt(DevDebug.style.width)-22) + 'px; height:' + (parseInt(DevDebug.style.height)-64) + 'px;">' + xObj.innerHTML + '</textarea>';
}

...
//    list($r,$g,$b,$alpha) = imagecolorsforindex($srcImage,$trans);
    ...
    $w = ($srcX*($zm+$cs))+4;
    $h = ($srcY*($zm+$cs))+4;
    fwrite($fhandle, "<table border=0 bgcolor=#CCCCCC width=$w cellpadding=0 cellspacing=$cs style=\"$bdr\">\n");
    for($y=0;$y<$srcY;$y++){
      fwrite($fhandle, "<tr>");
      for($x=0;$x<$srcX;$x++){
...
//        fwrite($fhandle, "<td width=$zm height=$zm onMouseDown=\"parent.doTool(this);\" colno=$color style=\"background-color:rgb(".$colors['red'].",".$colors['green'].",".$colors['blue'].");\" alpha=".$colors['alpha']." >");
        fwrite($fhandle, "<td width=$zm height=$zm onMouseDown=\"parent.doTool(this);\" colno=$color style=\"background-color:rgb(".$colors['red'].",".$colors['green'].",".$colors['blue'].");\" alpha=".$colors['alpha']." >");
      }
      fwrite($fhandle, "</tr>\n");
    }

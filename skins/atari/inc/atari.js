...
function WindowObject(xObjName,xObj,xHnd){
	...
	this.contentWidth = eval('' + xObjName + 'C.style.width');
	this.contentHeight = eval('' + xObjName + 'Body.style.height');
...
function MaxMin(xHnd){
	...
	if (xObj.style.width=='100%'){
		...
		eval('' + Windows[xHnd].ID + 'W.style.width = Windows[xHnd].width;');
		eval('' + Windows[xHnd].ID + 'W.style.height = Windows[xHnd].height;');
		eval('' + Windows[xHnd].ID + 'C.style.width = Windows[xHnd].contentWidth;');
		eval('' + Windows[xHnd].ID + 'Body.style.height = Windows[xHnd].contentHeight;');
		...
		eval('' + Windows[xHnd].ID + 'W.style.width = "100%";');
		eval('' + Windows[xHnd].ID + 'W.style.height = height-25 + "px";');
		eval('' + Windows[xHnd].ID + 'C.style.width = width-62;');
		eval('' + Windows[xHnd].ID + 'Body.style.height = height-25-65;');
...
function doMover(eObj){
	...
	if (ReSize && (ReSizeWindow!=-1)) {
		...
		eval('' + Windows[ReSizeWindow].ID + 'W.style.width = tmpX + "px";');
		eval('' + Windows[ReSizeWindow].ID + 'W.style.height = tmpY + "px";');
		eval('' + Windows[ReSizeWindow].ID + 'C.style.width = (tmpX-62) + "px";');
		eval('' + Windows[ReSizeWindow].ID + 'Body.style.height = (tmpY-65) + "px";');
...
function doEnd(eObj){
	...
	if (ReSize && (ReSizeWindow!=-1)){
		...
		Windows[ReSizeWindow].contentWidth = eval('' + Windows[ReSizeWindow].ID + 'C.style.width;');
		Windows[ReSizeWindow].contentHeight = eval('' + Windows[ReSizeWindow].ID + 'Body.style.height;');
...
// 									-= Window MouseWheel =-
function handle(xHowMuch,xObj,mObj,xShift) {
	if (xShift)
		scrollIt = setTimeout('scrollClick("'+xHowMuch+'",'+xObj.id+'ScrollH,'+mObj.id+')',125);
	else
		scrollIt = setTimeout('scrollClick("'+xHowMuch+'",'+xObj.id+'ScrollV,'+mObj.id+')',125);
}

function doMouseWheel(xHowMuch,eObj,xObj,mObj){
	var delta = 0;
	if (!eObj) eObj = window.event;
	if (eObj.wheelDelta) {
		delta = eObj.wheelDelta/120; 
		if (window.opera) delta = -delta;
	} else if (eObj.detail) {
		delta = -eObj.detail/3;
	}
	if (delta!=0)
		handle(-(delta*xHowMuch),xObj,mObj,eObj.shiftKey);
...
function storeWindows(){
  ...
  for (i=0; i<WindowNext; i++){
	...
	xWindows = xWindows + xW.contentWidth + ',';
	xWindows = xWindows + xW.contentHeight + ',';
...
//alert(xCend.toString()+':'+xSettings)
  xWs = xSettings.split('|');
  ...
  for(i=1;i<xWs.length;i++){
    ...
    xW.width = xS[3];
    xW.height = xS[4];
    xW.contentWidth = xS[5];
    xW.contentHeight = xS[6];
